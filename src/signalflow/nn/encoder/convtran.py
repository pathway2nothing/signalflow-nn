...
class ConvTranEncoder(nn.Module, SfTorchModuleMixin):
    ...
    def tune(cls, trial, model_size: str = "small") -> dict:
        ...
        d_model = trial.suggest_categorical("convtran_d_model", config["d_model"])
        nhead = trial.suggest_categorical("convtran_nhead", [2, 4, 8])
        ...
        return {
            ...
            "d_model": d_model,
            "nhead": nhead,
            "num_layers": trial.suggest_int("convtran_num_layers", *config["layers"]),
            "dim_feedforward": trial.suggest_categorical("convtran_ffn", config["ffn"]),
            "conv_kernel_size": trial.suggest_categorical("convtran_conv_kernel", [3, 5, 7, 11]),
            "dropout": trial.suggest_float("convtran_dropout", 0.0, 0.5),
        }
